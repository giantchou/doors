...
function GetInfoadd() {
    var domae = window.location.host;
    $.post("/ajax.ashx?action=showaddr&t=" + Math.random(), {
        domae: domae
    }, function (msg) {
        var sMsg = gav(msg, "msg");
        var sta = gav(msg, "state");
   ...
   $(function () {
       ...
       */
       ...
       for (var i = count - 1; i >= 0; i--) {
           ...
           //主导航的匹配链接
           var $a = $("ul.container a[href*='" + current + "']");

           //主导航的匹配文字  [一般情况下是没有用的,有时候会碰到说是这个文字,但是链接确是另外一个,但是导航选中还是要加在文字上面]
           //比如 二级分类是成功案例,但是它下面还有一个三级分类是经典案例,主导航上的是二级分类确标着三级分类的链接,这个文字就有用了
           //况且这里还要先判断文字再判断链接
           var $t = $("ul.container a:contains('" + txt + "')");
       ...
       if (!$("ul.container li.cur").size()) {
           $("ul.nav a[href*='" + MARK + "']").parent("li").addClass("cur").siblings("li").removeClass("cur");
           //如果还没有主导航选中,给第一个加样式
           if (!$("ul.container li.cur").size()) {
               $("ul.container li:first").addClass("cur").siblings("li").removeClass("cur");
           }
       } else {
       }
